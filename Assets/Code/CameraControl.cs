...
public class CameraControl : MonoBehaviour
{
    ...
    // for camera restriction
    private float recentTerrainElevation;
    ...
    private void restrictCamera()
    {
        ...
        try
        {
            // region.getTileAt() can fail when camera is out of bounds
            recentTerrainElevation = 5;  //region.getTileAt(transform.position).coord.y;
        }
        ...
        float offsetAboveFloor = transform.position.y - recentTerrainElevation - minCameraToGroundDistance;
        float offsetBelowCeiling = recentTerrainElevation + maxCameraToGroundDistance - (transform.position.y);
